...
export const GET_N_LAST_RECORDS = `SELECT * FROM records WHERE maschine_id = ? ORDER BY date DESC LIMIT ? `;
export const GET_ALL_EXERCISES = `SELECT date FROM ex WHERE maschine_id = ?`;
export const INSERT_RECORD = `INSERT INTO records (ex_id, maschine_id, set_number, weight, reps, rir) VALUES (?,?,?,?,?,?)`;
